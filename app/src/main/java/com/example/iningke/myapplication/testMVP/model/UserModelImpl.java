...
public class UserModelImpl implements UserModel<UserInfoModel> {

    @Override
    public Subscription getUserInfo(String userId, IBaseRequestCallBack<UserInfoModel> callBack) {
        return RetrofitManager.getInstance(HostType.MOVIE_HOST)
                .getRetrofit().create(Api.class)
                .getUserInfo(userId)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new BaseSubscriber<UserInfoModel>(callBack));
    }
}
